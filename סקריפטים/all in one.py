...
def metadata(book_info: dict) -> dict:
    date = datetime(year=int(book_info["printYear"]), month=1, day=1)
    return {
        "heAuthors": [book_info["author"]],
        "title": book_info["displayName"],
        "enTitle": book_info["displayNameEnglish"],
        "authors": [book_info["authorEnglish"]],
        "pubDate": [int(book_info["printYear"])],
        "pubDateHeb": dates.HebrewDate.from_pydate(date).hebrew_year(thousands=True, withgershayim=True),
        "pubPlaceStringEn": book_info["printLocationEnglish"],
        "pubPlaceStringHe": book_info["printLocation"],
        "heCategories": [book_info["category"]],
        "categories": [book_info["categoryEnglish"]],
        "publisher": book_info["source"],
        "Sourcefolder": "Dicta"
    }
...
def get_new_books(new_json: list, old_json: list):
    # new_json = [{'displayName': book.get('displayName', ''),
    #              'OCRDataURL': book.get('OCRDataURL', '')} for book in new_json]
    old_json = [{'displayName': book.get('displayName', '')} for book in old_json]
    for book in new_json:
        relivenet_values = {"displayName": book.get("displayName", '')}
        if relivenet_values not in old_json:
            yield book
...
def main(url: str, old_json_path: str, target: str, csv_file_path: str, dif_folder: str) -> None:
    ...
    for book in get_new_books(new_json, old_books):
        ...
        all_metadata.append(metadata(book))
        print(f"Saving {target_file_path}...")
    ...
    print("Comparing with old list...")
    dif_books.main(dif_folder)
