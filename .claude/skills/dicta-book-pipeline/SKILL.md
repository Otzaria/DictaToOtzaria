...
6. **דיבור־המתחיל מודגש** — תחילת כל קטע: עטוף את **הלמה כולה** (לא רק מילה אחת) ב‑`<b>`. בפירושי תלמוד הלמה היא הציטוט מהגמרא, ומסתיימת בנקודה בתוך התג: `<b>ורבא אמר משום שאין עדים מצויים לקיימו.</b> פירוש...`. אַחֵד `<b>` עוקבים של למה/שם אחד (`<b>אמר</b> <b>המעתיק</b>` ⇒ `<b>אמר המעתיק</b>`); אל תאחד למות/מקורות נפרדים (`<b>תוס'</b> <b>ד"ה ולא</b>` נשארים שניים).
   - **כל מילה שאתה מעביר אל תחילת קטע — חייבת להיכנס לתוך ה‑`<b>`.** כשמעבירים סמן/ציון מהכותרת או משורת ההפניה הקודמת אל ראש שורת ההערה (למשל שם מפרש + `ד"ה`: `בלח"מ ד"ה`, `כסף משנה ד"ה`, `במשנה למלך ד"ה`), הקטע חייב להיפתח ב‑`<b>` שעוטף את המילים שהוספת **יחד עם** הלמה המקורית — לא להשאיר אותן כטקסט רגיל לפני ה‑`<b>`. נכון: `<b>בלח"מ ד"ה הפריש</b> מעוברת וילדה...`; שגוי: `בלח"מ ד"ה <b>הפריש</b> מעוברת וילדה...`. כלל זה תקף לכל הוספת מילים בתחילת קטע, לא רק לשמות מפרשים.
